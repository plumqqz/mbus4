...
Очередь создается функцией
  ...
  select mbus4.post_<qname>(data hstore,
   ...
   функцией
    ...
    путем применения для каждой успешной транзакции компенсационной транзакции. Например, создание тура для юзера - 
    это сага - надо

     1. Зарезервировать номер (вебсервис куда-то)
     2. Зарезервировать билеты на самолет (туда и обратно) исходя из параметров резервирования номера
     3. Исходя из параметров билетов - зарезервировать трансфер от/до аэропорта
    ...
    Создаются ДВЕ последовательности действий, две упорядоченных  группы сообщения : первая ("прямая") - резервирование 
    и вторая("обратная") - откат резервирования, причем вторая  упорядочена относительно сообщения в специальной очереди, 
    ...
    последовательности, полученные из этого служебнобного сообщения. Все сообщения прямой последовательности содержат 
    ссылку на головное сообщение последовательности отката.
 ...
 параллельно       \ 5      3                   Трансфер от отеля       1b                 | 5b     1b                  отмена резервирования трансфера до отеля      |
    ...
    туры, залипшие туры, отчетность по сформированным и т.п.), и вторая - компенсационные транзакции
     Псевдокод:
     -- forward transaction
     bung_iid := generate_new_msgiid('bungs');
     final_iid := generate_new_msgiid('final');
     ...
     post('bungs', id_to_take := array[ room_revrv, book_tiket_to, book_tiket_from, transf_to, transf_from, final_iid]);
     ...
     post('end_of_tour_reservation', id_to_take:=array[cancel_room_reservation,cancel_tiket_booking_to,canel_tiket_booking_from,canel_transfer_to,canel_transfer_from]);
